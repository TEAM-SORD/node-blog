...
describe("Main page (reading view)", function () {
    ...
    it("GET should respond with an OK status code", function (done) {
    ...
    it("POST should respond with an OK status code", function (done) {

        request.method = "POST";
        shot.inject(server.create, request, function (res) {
            assert.equal(res.statusCode, 200);
            done();
        });

    });
    it("PUT should respond with an OK status code", function (done) {

        request.method = "PUT";
        shot.inject(server.update, request, function (res) {
            assert.equal(res.statusCode, 200);
            done();
        });

    });
    it("DELETE should respond with an OK status code", function (done) {

        request.method = "DELETE";
        shot.inject(server.delete, request, function (res) {
            assert.equal(res.statusCode, 200);
            done();
        });
    ...
    //     shot.inject(server.read, request, function (res) {
    //         // Placeholder test - looking for evidence of 'blog post' html
    //         assert.equal(res.payload.match(/blog/), true);
    //         done();
    //     });
